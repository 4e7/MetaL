...
namespace consoleTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Лексемы в строке :");
            foreach (Lexema l in lexems)
            {
                Console.WriteLine("\t" + l.Data + " Тип :" + l.GetType().Name);
            }
            if (lm.errors.Count > 0)
            {
                Console.WriteLine("Обнаружен неизвестный код!");
                foreach (LexemError e in lm.errors)
                {
                    Console.WriteLine("Неопознанная лексема " + e.lexem + " в  позиции " + e.position.ToString());
                }
            }
        }
    }
